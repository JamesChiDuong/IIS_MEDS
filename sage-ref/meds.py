...
class MEDSbase:
  ...
  def crypto_sign_keypair(self):
    ...
    for i in range(_sage_const_1 , s):
      while True: # repeat until A[i] and B[i] are invertible and G[i] has systematic form
        ...
        Amm = ExpandFqs(sigma_alpha, _sage_const_1 , GFq)[_sage_const_0 ]
        logging.debug(f"Amm:\n{Amm}")
        ...
        check_A_i, check_B_i = solve_symb([matrix(GFq, m, n, G0prime.rows()[i]) for i in range(_sage_const_2 )], Amm)
  ...
  def crypto_sign(self, sk, msg):
    ...
    for i in range(t):
      while True:
        sigma_prime_i = alpha + sigma[i] + i.to_bytes(_sage_const_4 , "little")
  ...
  def crypto_sign_open(self, pk, ms):
    ...
    l_Gi = ceil(((k-_sage_const_2 )*(m*n-k) + n) * GF_BITS / _sage_const_8 )
    ...
    munu = ms[:(ceil(m*m * GF_BITS / _sage_const_8 ) + ceil(n*n * GF_BITS / _sage_const_8 ))*w]
    ms = ms[(ceil(m*m * GF_BITS / _sage_const_8 ) + ceil(n*n * GF_BITS / _sage_const_8 ))*w:]
...
if __name__ == "__main__":
  ...
  # Test and benchmark:
  try:
    randombytes_init(bytes([_sage_const_0 ]*_sage_const_48 ), None, _sage_const_256 )
